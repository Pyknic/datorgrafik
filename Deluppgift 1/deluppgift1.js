...
// en webbläsare, förslagsvis Firefox.
//
// Tips:
...
//
// Notera att raderna separeras med komma ',' och inte semikolon ';'.
...
function createSquare()
{
    ...
    for (var i = 0; i < 6; i++) { // Gray
        ...
        // att bli 6*4=24, medan antalet värden i positions-arrayen är
        // 6*3=18.
...
function createHouse2() {
    ...
    // för en ARRAY_BUFFER. Det betyder att den lagrar index till andra arrayer.
    // Typen är även Uint16Array (16-bitars unsigned int) istället för
    // Float32Array.
    ...
    // Förra gången lagrade vi antalet trianglar. När man målar ut indexerade
    // primitiver behöver man istället ange hur många index man har. Just nu är
    // det 6 st, men när du är färdig med den här funktionen kommer det att vara
    // 16*3=48.
...
// Den här funktionen målar ut scenen. Varje gång du vill måla ut ett nytt
// objekt (ett nytt draw-call) så måste du förbereda P*V*W-matrisen som du vill
...
function drawScene(time) {
    ...
    // Om vi därefter multiplicerar matrisen med en position så kommer
    // positionen flyttas nedåt 20 steg.
    ...
    //       matrisen till grafikkortet innan rendering.
...
// du istället i funktionen createSquare() som ligger längre upp.
function drawSquare()
{
    gl.bindBuffer(gl.ARRAY_BUFFER, shared.square.positionBuffer);
...
// Den här funktionen målar ut de semi-transparenta planen. Du måste
// implementera den på egen hand. Du kommer att behöva använda funktionerna
// gl.disable(), gl.blendFunc() och gl.enable() på rätt sätt.
// 
...
// gl_Position  Ett speciellt returvärde i vertexShadern som berättar vart
//              hörnet skall målas ut i vy-rymd.
